...
def test_is_palindrome_string_correct():
    my_string = "abccba"
    assert string_operations.check_if_sequence_is_palindrome(my_string), print("String is not a palindrome")


def test_is_palindrome_string_incorrect():
    my_string = "Test123"
    assert not string_operations.check_if_sequence_is_palindrome(my_string), print("String should not be a palindrome")


def test_palindrome_string_empty():
    my_string = ""
    assert string_operations.check_if_sequence_is_palindrome(my_string)


def test_palindrome_array_of_numbers_correct():
    my_array = [1, 2, 3, 3, 2, 1]
    assert string_operations.check_if_sequence_is_palindrome(my_array)


def test_palindrome_array_of_numbers_incorrect():
    my_array = [1, 2, 4, 3, 2, 1]
    assert not string_operations.check_if_sequence_is_palindrome(my_array)
...
def test_is_number_between_correct():
    ...
    left = 10
    right = 200
    assert numbers_operations.check_if_number_is_between(left, right, number)
...
def test_is_number_between_check_right_boundary():
    ...
    left = 10
    right = 200
    assert not numbers_operations.check_if_number_is_between(left, right, number)
